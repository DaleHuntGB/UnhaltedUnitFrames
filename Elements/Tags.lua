...
local function CreateUnitTag(unitFrame, unit, tagDB)
    ...
    if not unitFrame.Tags[tagDB] then
        unitFrame.Tags[tagDB] = unitFrame.HighLevelContainer:CreateFontString(UUF:FetchFrameName(unit, unitFrame) .. "_" .. tagDB, "ARTWORK")
    ...
    for unit in pairs(UUF.db.profile.Units) do
        ...
        elseif unit == "party" then
            for _, partyFrame in ipairs(UUF.PARTY_FRAMES) do
                for tagName in pairs(UnitDB.Tags) do UUF:UpdateUnitTag(partyFrame, unit, tagName) end
            end
        else
