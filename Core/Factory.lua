...
local function CreateCastBar(unitFrame, unit, DB, GeneralDB)
    if not DB.CastBar then return end
    if unit:match("^boss(%d+)$") then local id = tonumber(unit:match("%d+")) if id > 5 then return end end
    local CastBarContainer = CreateFrame("Frame", "UUF_" .. unit .. "_CastBarContainer", UIParent, "BackdropTemplate")
    CastBarContainer:SetSize(DB.CastBar.Width, DB.CastBar.Height)
    CastBarContainer:SetBackdrop({ bgFile = "Interface\\Buttons\\WHITE8X8", edgeFile = "Interface\\Buttons\\WHITE8X8", edgeSize = 1, })
    CastBarContainer:SetBackdropColor(26/255, 26/255, 26/255, 1.0)
    CastBarContainer:SetBackdropBorderColor(0, 0, 0, 1)
    CastBarContainer:ClearAllPoints()
    CastBarContainer:SetPoint(DB.CastBar.AnchorFrom, unitFrame, DB.CastBar.AnchorTo, DB.CastBar.XPosition, DB.CastBar.YPosition)

    local CastBarIcon = CastBarContainer:CreateTexture("UUF_" .. unit .. "_CastBarIcon", "ARTWORK")
    CastBarIcon:SetSize(DB.CastBar.Height - 2, DB.CastBar.Height - 2)
    CastBarIcon:SetPoint("LEFT", CastBarContainer, "LEFT", 1, 0)
    CastBarIcon:SetTexCoord(0.07, 0.93, 0.07, 0.93)

    local CastBar = CreateFrame("StatusBar", "UUF_" .. unit .. "_CastBar", CastBarContainer)
    ...
    CastBar:SetStatusBarColor(DB.CastBar.FGColour[1], DB.CastBar.FGColour[2], DB.CastBar.FGColour[3], DB.CastBar.FGColour[4] or 1)

    CastBar.SpellName = CastBar:CreateFontString(nil, "OVERLAY")
    CastBar.SpellName:SetFont(UUF.Media.Font, DB.CastBar.Text.SpellName.FontSize, GeneralDB.FontFlag)
    CastBar.SpellName:SetPoint(DB.CastBar.Text.SpellName.AnchorFrom, CastBar, DB.CastBar.Text.SpellName.AnchorTo, DB.CastBar.Text.SpellName.OffsetX, DB.CastBar.Text.SpellName.OffsetY)
    CastBar.SpellName:SetText("")
    CastBar.SpellName:SetJustifyH("LEFT")
    CastBar.SpellName:SetTextColor(DB.CastBar.Text.SpellName.Colour[1], DB.CastBar.Text.SpellName.Colour[2], DB.CastBar.Text.SpellName.Colour[3], DB.CastBar.Text.SpellName.Colour[4] or 1)

    CastBar.Duration = CastBar:CreateFontString(nil, "OVERLAY")
    CastBar.Duration:SetFont(UUF.Media.Font, DB.CastBar.Text.Time.FontSize, GeneralDB.FontFlag)
    CastBar.Duration:SetPoint(DB.CastBar.Text.Time.AnchorFrom, CastBar, DB.CastBar.Text.Time.AnchorTo, DB.CastBar.Text.Time.OffsetX, DB.CastBar.Text.Time.OffsetY)
    CastBar.Duration:SetText("")
    CastBar.Duration:SetJustifyH("RIGHT")
    CastBar.Duration:SetTextColor(DB.CastBar.Text.Time.Colour[1], DB.CastBar.Text.Time.Colour[2], DB.CastBar.Text.Time.Colour[3], DB.CastBar.Text.Time.Colour[4] or 1)

    CastBar.Icon = CastBarIcon

    CastBar:SetScript("OnShow", function(self)
        local spellName, _, icon = UnitCastingInfo(unit)
        if not spellName then spellName, _, icon = UnitChannelInfo(unit) end
        if icon then
    ...
    unitFrame.CastBarContainer = CastBarContainer
    CastBar:Hide()
...
function UUF:CreateUnitFrame(unit)
    ...
    -- unitFrame.castBar = CreateCastBar(unitFrame, unit, DB, GeneralDB)
