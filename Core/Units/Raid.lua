...
function UUF:GetRaidGroupFilter()
	...
	for _, groupIndex in ipairs(FrameDB.GroupingOrder) do
		...
		if groupIndex and groupIndex >= 1 and groupIndex <= 8 and not selectedGroups[groupIndex] then
	...
	for groupIndex = #groupingOrder + 1, 8 do groupingOrder[groupIndex] = 0 end
...
function UUF:SpawnRaidFrames()
	...
	local headerAttributes = {
		...
		showPlayer = true,
		showSolo = false,
		groupFilter = UUF:GetRaidGroupFilter(),
		...
		maxColumns = math.ceil(40 / FrameDB.UnitsPerColumn),
	...
	if FrameDB.GroupBy == "GROUP" then
		...
		headerAttributes.groupingOrder = UUF:GetRaidGroupFilter()
	end
	...
	UUF.RAID:SetAttribute("startingIndex", -39)
	UUF.RAID:Show()
...
function UUF:UpdateRaidFrames()
	...
	if UUF.RAID:GetAttribute("maxColumns") ~= math.ceil(40 / FrameDB.UnitsPerColumn) then UUF.RAID:SetAttribute("maxColumns", math.ceil(40 / FrameDB.UnitsPerColumn)) end
